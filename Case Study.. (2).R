transaction=read.csv("C:\\Users\\SHAURYA\\Desktop\\R programming\\R case study 1 (Retail)\\Transactions.csv")
customer=read.csv("C:\\Users\\SHAURYA\\Desktop\\R programming\\R case study 1 (Retail)\\Customer.csv")
product=read.csv("C:\\Users\\SHAURYA\\Desktop\\R programming\\R case study 1 (Retail)\\prod_cat_info.csv")
getwd()
library(dplyr)

# Ques1. Merge all the datasets as Consumer_Final
trans_prod<-left_join(transaction,product,by=c("prod_cat_code","prod_subcat_code" = "prod_sub_cat_code"))
Consumer_Final=merge(trans_prod,customer,by="cust_id")
names(Consumer_Final)
# Ques2(a) Get the column names and thier corresponding data types  

table_datatypes=sapply(Consumer_Final, class)

# Ques2(b) Get the top/bottom 10 observations
Top10<-head(Consumer_Final,5)
Bottom10<-tail(Consumer_Final,5)
Top_Bottom10<-merge(Top10,Bottom10)
# Ques2(c) Five number summary for the dataset
Five_number_summary<-summary(Consumer_Final)
# Ques2(d) Frequency table for all catagorical variables
Frequency1<-Consumer_Final%>%group_by(prod_cat_code)%>%summarise(Total=n())
...
Frequency4<-Consumer_Final%>%group_by(prod_cat)%>%summarise(Total=n())
Frequency5<-Consumer_Final%>%group_by(prod_sub_cat_code)%>%summarise(Total=n())
Frequency6<-Consumer_Final%>%group_by(prod_subcat)%>%summarise(Total=n())
Frequency7<-Consumer_Final%>%group_by(Gender)%>%summarise(Total=n())
Frequency8<-Consumer_Final%>%group_by(city_code)%>%summarise(Total=n())
...
Consumer_Final$month<-month(Consumer_Final$tran_date)
Consumer_Final$day<-day(Consumer_Final$tran_date)
span<-interval(ymd("2011-01-25"),ymd("2014-02-28"))
duration<-as.duration(span)
library(lubridate)

interval(ymd("2011-01-"))
# Ques5
